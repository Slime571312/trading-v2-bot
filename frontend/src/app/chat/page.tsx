...
const MODELS: { value: ChatModel; label: string; hint: string }[] = [
  { value: "claude-sonnet-4-6", label: "Sonnet 4.6", hint: "Default — schnell" },
  { value: "claude-opus-4-7", label: "Opus 4.7", hint: "Tiefer, langsamer" },
  { value: "claude-haiku-4-5-20251001", label: "Haiku 4.5", hint: "Schnellster" },
];
...
/**
 * Konvertiert die rohen API-Messages in vereinfachte UI-Zeilen.
 * Anthropic-Messages haben Content-Blocks; wir extrahieren Text + Tool-Calls
 * und gruppieren so dass jede Assistant-Antwort eine Zeile ist (inkl. ihrer Tools).
 */
function detailToRows(detail: ConversationDetail): ChatRow[] {
  const rows: ChatRow[] = [];
  let pendingToolUses: { id: string; name: string; input: Record<string, unknown> }[] = [];

  for (const m of detail.messages) {
    if (m.role === "user") {
      // User-Message: entweder String (initial), oder Array von tool_results
      if (typeof m.content === "string") {
        rows.push({ role: "user", text: m.content, tools: [] });
      } else if (Array.isArray(m.content)) {
        for (const block of m.content) {
          if ((block as { type: string }).type === "tool_result") {
            const tr = block as { tool_use_id: string; content: string };
            const matched = pendingToolUses.find((tu) => tu.id === tr.tool_use_id);
            if (matched && rows.length > 0) {
              // Letzten Assistant-Row holen, Tool drauflegen
              const last = rows[rows.length - 1];
              if (last.role === "assistant") {
                last.tools.push({
                  name: matched.name, input: matched.input,
                  output: tr.content, iteration: 0,
                });
              }
            }
          }
      ...
      const localTools: { id: string; name: string; input: Record<string, unknown> }[] = [];
      for (const block of m.content) {
        ...
        if (t === "text") {
          ...
          localTools.push({ id: tu.id, name: tu.name, input: tu.input });
        }
      }
      rows.push({ role: "assistant", text: textParts.join("\n"), tools: [] });
      pendingToolUses = localTools;
    }
  }
  return rows;
}
...
function ProposalsPanel({ onChange }: { onChange: () => void }) {
  ...
  if (proposals.length === 0) {
    return (
      <div style={{ ...sidebarCard, color: "var(--text-dim)", fontSize: 12 }}>
        ...
        Frag den Chat z.B.: <i>„Schlag mir eine bessere RR-Schwelle für BTC vor"</i>
      </div>
    );
  }
...
export default function ChatPage() {
  ...
  const [proposalsRefreshTrigger, setProposalsRefreshTrigger] = useState(0);
  ...
  // Active Conversation laden wenn ID wechselt
  useEffect(() => {
  ...
  // Auto-Scroll nach Update
  useEffect(() => {
    if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
  }, [activeDetail, lastTools, busy]);
  ...
  async function handleSend() {
    ...
    setLastTools([]);
    try {
      const res = await sendChatMessage(message, activeId ?? undefined, model);
      setActiveId(res.conversation_id);
      // Reload conversation
      const d = await getConversation(res.conversation_id);
      setActiveDetail(d);
      setLastTools(res.tool_calls);
      await loadConversations();
      // Trigger proposals refresh in case Claude added one
      setProposalsRefreshTrigger((n) => n + 1);
    } catch (e) {
      setError(String(e));
    } finally {
      setBusy(false);
  ...
  const rows: ChatRow[] = activeDetail ? detailToRows(activeDetail) : [];
  ...
  return (
    <div style={{ display: "flex", gap: 16, height: "calc(100vh - 100px)" }}>
      ...
      <aside style={{ width: 220, display: "flex", flexDirection: "column", gap: 12, flexShrink: 0 }}>
            ...
            >
              <div style={{ overflow: "hidden", flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>
                  {c.n_messages} msgs · {c.model.split("-").slice(-2, -1).join("-")}
                </div>
              </div>
              <button
        ...
        <ProposalsPanel key={proposalsRefreshTrigger} onChange={loadConversations} />
      </aside>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <div style={{
          ...
          <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>
            {activeDetail ? activeDetail.title : "Chat-Bot"}
          </h1>
          <select
            value={model}
            ...
            style={{
              ...
              borderRadius: 6, color: "var(--text)", padding: "5px 10px", fontSize: 12,
            }}
          >
        ...
        >
          ...
          {rows.map((row, i) => (
            <div key={i} style={{ display: "flex", flexDirection: "column" }}>
              <div style={{
                ...
                background: row.role === "user" ? "var(--accent-dim)" : "var(--bg)",
                color: row.role === "user" ? "#fff" : "var(--text)",
                padding: "10px 14px", borderRadius: 12,
                maxWidth: "85%", whiteSpace: "pre-wrap", fontSize: 14, lineHeight: 1.5,
              }}>
                {row.text || (row.role === "assistant" && row.tools.length > 0 ? "(Tool-Calls — siehe unten)" : "")}
              </div>
              {row.tools.length > 0 && (
                <div style={{ alignSelf: "flex-start", maxWidth: "85%", marginTop: 4 }}>
                  {row.tools.map((t, j) => (
                    <ToolUseBadge key={j} name={t.name} input={t.input} output={t.output} />
                  ))}
                </div>
              )}
            </div>
          ...
          {busy && (
            <div style={{
              ...
              ● ● ● Claude denkt nach (kann bei Tool-Loop einige Sekunden dauern)...
            </div>
          )}
        </div>
